<template>
  <div v-if="editor">
    ...
    <drag-handle :editor="editor" :on-node-change="handleNodeChange">
      <div class="custom-drag-handle">{{ nodePos }}</div>
    </drag-handle>
  </div>
...
export default {
  ...
  methods: {
    handleNodeChange(data: { node: TiptapNode | null; pos: number }) {
      if (data.node) {
        this.node = data.node;
        console.log("🚀 ~ handleNodeChange ~ this.node: ", this.node);
      }
      this.nodePos = data.pos;
    },
    // isTextSelectionValid(editor) {
    //   console.log('checking text selection validity')
    ...
    //   return isValid
    // },
    // selectNodeAndHideFloating(editor, pos) {
    //   console.log('selecting node and hiding floating toolbar')
    //   if (!editor) return
    ...
    //
    // },
    // onClickDragHandle(event) {
    //   console.log('handle drag handle click')
    //   this.selectNodeAndHideFloating(this.editor, this.nodePos)
    //
    // },
  },
  setup() {},
